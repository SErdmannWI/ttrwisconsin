package com.serdmannwi.practiceprograms.tickettoridewisconsin.constants;

public class ActionConstants {

    /**--------------------------------------------------- Names ---------------------------------------------------**/
    public static final String PICK_CARDS = "Choose from face-up or face-down cards";
    public static final String PICK_LOCOMOTIVE = "Choose 1 face-up locomotive card";
    public static final String CLAIM_ROUTE = "Claim a Route";
    public static final String USE_ABILITY = "Use Freight Station Ability";
    public static final String RESOLVE_EVENT = "Resolve ongoing event";
    public static final String CHOOSE_FREIGHT_CONTRACTS = "Choose more Freight Contracts";
    public static final String CANCEL_FREIGHT_CONTRACT = "Cancel Freight Contract";

    /**---------------------------------------------------- IDs ----------------------------------------------------**/
    public static final String PICK_CARDS_ID = "Action1";
    public static final String PICK_LOCOMOTIVE_ID = "Action2";
    public static final String CLAIM_ROUTE_ID = "Action3";
    public static final String USE_ABILITY_ID = "Action4";
    public static final String RESOLVE_EVENT_ID = "Action5";
    public static final String CHOOSE_FREIGHT_CONTRACTS_ID = "Action6";
    public static final String CANCEL_FREIGHT_CONTRACT_ID = "Action7";
}
